...
Purpose:
    Run the Restock at depot algorithm that solves the VeRolog problem
...
def decideScheduleTech(k,Routes,constraints,requests,machines,numberRequests,numberMachines,numberLocations,technicians,numberTechnicians,techniciansCluster,Graph,availableTechSched):
    ...
    while day <= constraints['DAYS']:
        ...
        while requestsDay != []:
            ...
            #find the available techs (ID) on day, based on consecutive working days constraint
...
###########################################################
### called in calcDeliveryCost(), combineRoutes()
...
def combineRoutes(scheduleCombined,Graph,constraints,requests,machines,numberRequests,numberMachines):
    ...
    Purpose:
        ...
        Note: requires installation of ortools, for the knapsack solver
    Input:
...
def main():
    ...
    scheduleDeliveryCombined = combineRoutes(scheduleCombined,Graph,constraints,requests,machines,numberRequests,numberMachines)
    ...
    #python SolutionVerolog2019.py -i VSC2019_ORTEC_early_13.txt -s TestSolutionFileRestock.txt
    return
